...
if __name__ == "__main__":
    ...
    add_extra_training_params(yolov9c_adam, optimizer="AdamW", name="Adam", epochs=epochs_adam, freeze=10)
    # Fine-tune
    yolov9c_adam_finetune = copy.deepcopy(train_params_c)
    add_extra_training_params(yolov9c_adam_finetune, optimizer="AdamW", name="Adam_finetuned", epochs=finetune, lr0=lr0 / 10)
    ...
    add_extra_training_params(yolov9c_sgd, optimizer="SGD", name="SGD", epochs=epochs_sgd, freeze=10)
    # Fine-tune
    yolov9c_sgd_finetune = copy.deepcopy(train_params_c)
    add_extra_training_params(yolov9c_sgd_finetune, optimizer="SGD", name="SGD_finetuned", epochs=finetune, lr0=lr0 / 10)
    ...
    add_extra_training_params(yolov9e_adam, optimizer="AdamW", name="Adam", epochs=epochs_adam, freeze=30)
    # Fine-tune
    yolov9e_adam_finetune = copy.deepcopy(train_params_e)
    add_extra_training_params(yolov9e_adam_finetune, optimizer="AdamW", name="Adam_finetuned", epochs=finetune, lr0=lr0 / 10)
    ...
    add_extra_training_params(yolov9e_sgd, optimizer="SGD", name="SGD", epochs=epochs_sgd, freeze=30)
    # Fine-tune
    yolov9e_sgd_finetune = copy.deepcopy(train_params_e)
    add_extra_training_params(yolov9e_sgd_finetune, optimizer="SGD", name="SGD_finetuned", epochs=finetune, lr0=lr0 / 10)
    ...
    # Fine-tune (cargar Shiny_SalmonV4_1024.pt de los entrenamientos anteriores y entrenarlos un poco más)
    ...
    #add_extra_training_params(yolov9e_seg_shiny, optimizer="SGD", name="SGD_2", epochs=50, freeze=30, model="models/training/yolov9e-seg/Salmones/SGD_2/weights/Shiny_SalmonV4_1024.pt")
    #yolov9c_seg_shiny =copy.deepcopy(train_params_c)
    #add_extra_training_params(yolov9c_seg_shiny, optimizer="AdamW", name="Adam_2", epochs=70, freeze=10, model="models/training/yolov9c-seg/Salmones/Adam_finetuned_2/weights/Shiny_SalmonV4_1024.pt")
    ...
    #    thread1 = Thread(target=thread_safe_training, args=(yolov9c_sgd))
    #    thread1.start() # SGD Modelo Large
    #    thread2 = Thread(target=thread_safe_training, args=(yolov9c_adam))
    #    thread2.start() # Adam Modelo Large
    #    thread3 = Thread(target=thread_safe_training, args=(yolov9e_sgd))
    #    thread3.start() # SGD Modelo X-Large
    #    thread4 = Thread(target=thread_safe_training, args=(yolov9e_adam))
    ...
    #    thread1_re = Thread(target=thread_safe_re_training, args=(modelo_c, yolov9c_sgd_finetune))
    #    thread1_re.start() # SGD Modelo Large
    #    thread2_re = Thread(target=thread_safe_re_training, args=(modelo_c, yolov9c_adam_finetune))
    #    thread2_re.start() # Adam Modelo Large
    #    thread3_re = Thread(target=thread_safe_re_training, args=(modelo_e, yolov9e_sgd_finetune))
    #    thread3_re.start() # SGD Modelo X-Large
    #    thread4_re = Thread(target=thread_safe_re_training, args=(modelo_e, yolov9e_adam_finetune))
