...
from training import get_training_params_for_datasets, add_extra_training_params, thread_safe_training, thread_safe_re_training
...
def experimento_2(modelo_c = "yolov9c-seg", modelo_e = "yolov9e-seg"):
    ...
    # Fine-tune
    yolov9c_adam_finetune = copy.deepcopy(train_params_c)
    add_extra_training_params(yolov9c_adam_finetune, optimizer="AdamW", name="Adam_finetuned_2", epochs=finetune, lr0=(lr0/10))
    for key, value in yolov9c_adam.items():
        yolov9c_adam_finetune[key]["model"] = os.path.join(value["project"], value["name"], "weights", "best.pt")
    ...
    # Fine-tune
    yolov9c_sgd_finetune = copy.deepcopy(train_params_c)
    add_extra_training_params(yolov9c_sgd_finetune, optimizer="SGD", name="SGD_finetuned_2", epochs=finetune, lr0=(lr0/10))
    for key, value in yolov9c_sgd.items():
        yolov9c_sgd_finetune[key]["model"] = os.path.join(value["project"], value["name"], "weights", "best.pt")
...
def train_shiny_salmons(best_model_c, best_model_e, modelo_c = "yolov9c-seg", modelo_e = "yolov9e-seg"):
    ...
    add_extra_training_params(yolov9c_sgd_shiny_finetune, optimizer="SGD", name="SGD_finetuned", epochs=20, lr0=lr0/10)
    ...
    add_extra_training_params(yolov9e_seg_shiny, optimizer="SGD", name="SGD_640", epochs=60, freeze=30, model=best_model_e)
    # Re-train
...
def experimento_4(modelo_c = "yolov9c", modelo_e = "yolov9e"):
    ...
    add_extra_training_params(train_params_c, lr0=lr0, batch=8, imgsz=640, single_cls=True, cos_lr=True, plots=True, patience=20)
    add_extra_training_params(train_params_e, lr0=lr0, batch=8, imgsz=640, single_cls=True, cos_lr=True, plots=True, patience=20)
